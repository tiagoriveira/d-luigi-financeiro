...
import { lancamentos, formatBRL } from "@/lib/mock-data";
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer } from "recharts";

const TIPO_LABELS: Record<string, string> = {
    FL: "Venda", MP: "Ingrediente", FX: "Despesa Fixa", VA: "Variável", TX: "Bancária", OP: "Operacional",
};
const TIPO_COLORS: Record<string, string> = {
    FL: "bg-success-light text-success",
    TX: "bg-[#FDF0E6] text-accent-warm",
    MP: "bg-[#EEF2FF] text-[#4F46E5]",
    FX: "bg-danger-light text-danger",
    VA: "bg-[#FDF4FF] text-[#9333EA]",
    OP: "bg-info-light text-info",
};
...
export default function CaixaPage() {
    ...
    const saldoContas = contas.map((c) => ({
        conta: c,
        saldo: lancamentos.filter((l) => l.conta === c).reduce((a, l) => a + l.valor, 0),
    }));

    // Resumo por tipo
    const tipos = Object.keys(TIPO_LABELS);
    const saldoTipos = tipos.map((t) => ({
        tipo: t,
        label: TIPO_LABELS[t],
        total: lancamentos.filter((l) => l.tipo === t).reduce((a, l) => a + Math.abs(l.valor), 0),
    })).filter((t) => t.total > 0);

    const FILTROS = [
        { label: "Todos", value: "todos" },
        { label: "Vendas", value: "FL" },
        { label: "Ingredientes", value: "MP" },
        { label: "Fixos", value: "FX" },
        { label: "Variáveis", value: "VA" },
    ];
            ...
            <div>
                ...
                <div className="bg-surface rounded-card border border-border shadow-sm overflow-hidden mb-4">
                    ...
                    <div className="grid gap-3 p-[18px_20px] items-end" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))" }}>
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[10.5px] font-semibold tracking-[0.06em] uppercase text-text-3">Data</label>
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <input type="text" value={novoLanc.descricao} onChange={(e) => setNovoLanc({ ...novoLanc, descricao: e.target.value })} placeholder="Ex: Vendas em Dinheiro"
                                className="px-[11px] py-2 border border-border-strong rounded-sm text-[13px] text-text bg-surface outline-none focus:border-accent transition-all" />
                        </div>
                        <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <select value={novoLanc.conta} onChange={(e) => setNovoLanc({ ...novoLanc, conta: e.target.value })}
                                ...
                                <option>Sicoob PJ</option>
                                <option>Santander</option>
                            </select>
                        </div>
                        <div className="flex items-end">
                            <button className="inline-flex items-center gap-1.5 px-[15px] py-2 rounded-sm text-[12.5px] font-medium bg-accent text-white hover:bg-[#245A3C] transition-all w-full justify-center">
                                Lançar
                            </button>
                        </div>
                    </div>
                ...
                <div className="bg-surface rounded-card border border-border shadow-sm overflow-hidden mb-[18px]">
                    <div className="p-[16px_20px_12px] flex items-center justify-between border-b border-border flex-wrap gap-2">
                        <div className="text-[13.5px] font-semibold text-text">Extrato do Mês</div>
                        <div className="flex gap-1.5 flex-wrap">
                            {FILTROS.map((f) => (
                                <button key={f.value} onClick={() => setFiltroAtivo(f.value)}
                                    className={`px-2 py-0.5 rounded text-[10.5px] font-medium border transition-all ${filtroAtivo === f.value ? "bg-surface-3 border-border-strong text-text" : "bg-transparent border-border-strong text-text-2 hover:bg-surface"}`}>
                                    {f.label}
                                </button>
                            ))}
                        </div>
                                ...
                                {extrato.map((l) => (
                                    <tr key={l.id} className="hover:bg-surface-2">
                                        <td className="px-5 py-[10px] border-b border-border">
                                            <span className={`inline-flex items-center justify-center w-[26px] h-[26px] rounded-[5px] text-[9px] font-bold tracking-[0.04em] ${TIPO_COLORS[l.tipo] || "bg-surface-2 text-text-2"}`}>
                                                {l.tipo}
                                            </span>
                                        </td>
            ...
            <div>
                <div className="bg-surface rounded-card border border-border shadow-sm overflow-hidden">
                    ...
                    <div className="text-center px-[18px] py-[22px] border-b border-border">
                        ...
                        <div className={`font-serif text-[38px] font-light tracking-[-1px] ${saldoTotal >= 0 ? "text-success" : "text-danger"}`}>
                        ...
                        {saldoContas.map((c, i) => (
                            <div key={c.conta} className="flex justify-between items-center py-[10px] border-b border-border last:border-b-0 text-[12.5px]">
                                <div className="flex items-center gap-2">
                                    <span className="w-[7px] h-[7px] rounded-full" style={{ background: CONTAS_COLORS[i % CONTAS_COLORS.length] }} />
                                    {c.conta}
                                </div>
